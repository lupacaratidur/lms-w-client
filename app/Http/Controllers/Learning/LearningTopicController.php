...
class LearningTopicController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Ambil mata pelajaran dari dropdown (jika ada)
        $selectedSubject = $request->input('subject_id', null);
    
        // Query untuk mendapatkan daftar topik
        if ($role == "Pengajar") {
            $query = LearningTopic::with(['subject', 'user', 'grades'])->where('user_id', Auth::id());
        } else {
            $query = LearningTopic::with(['subject', 'user', 'grades']);
        }
    
        // Filter by subject jika dipilih
        if ($selectedSubject) {
            $query->where('subject_id', $selectedSubject);
        }
    
        // Dapatkan daftar topik
        $topics = $query->get();
    
        // Ambil daftar semua mata pelajaran untuk dropdown
        $subjects = Subject::all(); // Asumsi ada model Subject
    
        return view('admin.learning-topic.index', compact('topics', 'subjects', 'selectedSubject'));
    }
